...
const requestHandler = request => {
  if (isHandlerEnabled(request)) {
    if (auth.isAuthenticated() && request.url !== API_REFRESH_URL && request.url !== API_SIGNIN_URL) {
...
const createAxiosResponseInterceptor = () => {
  const interceptor = axios.interceptors.response.use(
    ...
    error => {
      ...
      return axios({ url: API_REFRESH_URL, method: "POST", withCredentials: true }).then(response => {
